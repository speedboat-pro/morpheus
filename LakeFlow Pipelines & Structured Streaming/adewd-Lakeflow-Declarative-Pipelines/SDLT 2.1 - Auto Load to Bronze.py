...
# MAGIC |---|
# MAGIC |[Auto Load to Bronze]($./Pipeline/SDLT 2.1.1 - Auto Load to Bronze)|
# MAGIC
...
# MAGIC
# MAGIC **NOTE:** Re-running the entire notebook will delete the underlying data files for both the source data and your pipeline.
...
# MAGIC Run the cell below to auto-generate your pipeline using the provided configuration values.
# MAGIC Please navigate to **Jobs and Pipelines** to view the pipeline.
# MAGIC
...
demo_pipeline = DeclarativePipelineCreator(
    pipeline_name=f'demo_01_pipeline_{DA.schema_name}', 
    catalog_name='dbacademy',
    schema_name=DA.schema_name,
    root_path_folder_name='Pipeline',
    source_folder_names=['SDLT 2.1.1 - Auto Load to Bronze'],
